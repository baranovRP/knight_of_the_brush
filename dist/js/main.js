...
/***/ function(module, exports, __webpack_require__) {
	...
	var _playfield = __webpack_require__(299);
	
	var _playfield2 = _interopRequireDefault(_playfield);
	
	function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }
	
	var field = new _playfield2.default(50); /* eslint no-use-before-define: ["error", { "functions": false }] */
...
/***/ function(module, exports, __webpack_require__) {
	...
	function _toConsumableArray(arr) { if (Array.isArray(arr)) { for (var i = 0, arr2 = Array(arr.length); i < arr.length; i++) { arr2[i] = arr[i]; } return arr2; } else { return Array.from(arr); } } /* eslint no-use-before-define: ["error", { "functions": false }] */
	/* eslint no-global-assign: "warn" */
	/* eslint-env browser */
	
	var counter = 0;
	
	function Playfield(radius) {
	  var size = 10;
	  this.radius = radius;
	...
	Playfield.prototype.render = function render(l) {
	  var self = this.node;
	  self.innerHTML = '';
	  [].concat(_toConsumableArray(l)).forEach(function (item) {
	    return self.appendChild(item.node);
	  });
	};
	...
	Playfield.prototype.removeCircle = function addCircle(circle) {
	  var idToRemove = circle.getAttribute('id');
	  this.circles = this.circles.filter(function (i) {
	    return i.node.getAttribute('id') !== idToRemove;
	  });
	...
	Playfield.prototype.fieldEvents = function fieldEvents() {
	  ...
	  this.node.addEventListener('dblclick', function (event) {
	    ...
	    if (target.classList.contains('circle')) {
	      _this.removeCircle(event.target);
	    } else if (target === _this.node) {
	      var center = { x: event.clientX, y: event.clientY };
	      if (!isCrossed(_this.circles, center, self.radius * 2)) {
	        _this.addCircle(new _circle2.default(center, counter++, self.radius));
	      }
	    }
	  });
	  this.node.addEventListener('circlemove', function (event) {
	    var circle = event.detail.circle;
	    var list = _this.circles.filter(function (i) {
	      return i !== circle;
	    });
	
	    while (isCrossed(list, circle.coordinates, self.radius / 2)) {
	      var nearestCircle = list.reduce(function (previous, current) {
	        var minDist = calcDistance(previous.coordinates, circle.coordinates);
	        var currDist = calcDistance(current.coordinates, circle.coordinates);
	      ...
	      if (self.radius / 2 > calcDistance(nearestCircle.coordinates, circle.coordinates)) {
	        ...
	        _this.removeCircle(nearestCircle.node);
	        circle.setStyles();
	        list = _this.circles.filter(function (i) {
	          return i !== circle;
	        });
	      }
	    }
	...
	function mixColors(color1, color2) {
	  ...
	  var avg = Math.round((toDex(color1.slice(1)) + toDex(color2.slice(1))) / 2);
	...
	function init(field, length, limit) {
	  ...
	  while (coords.length < length) {
	    var center = randomCoordinates(field);
	    if (!isCrossed(coords, center, limit)) {
	      coords.push(new _circle2.default(center, counter++, field.radius));
	    }
	  }
	...
	function randomCoordinates(field) {
	  var width = field.node.clientWidth;
	  var height = field.node.clientHeight;
	
	  var newX = randomInt(field.radius, width - field.radius);
	  var newY = randomInt(field.radius, height - field.radius);
	...
	function isCrossed(l, center, limit) {
	  ...
	  return l.some(function (el) {
	    return limit > calcDistance(el.coordinates, center);
	  });
	}
...
/***/ function(module, exports) {
	...
	function Circle(coords, idx, radius) {
	...
	Circle.prototype.circleEvents = function move() {
	  ...
	  self.node.addEventListener('mousedown', function (event) {
	    var el = event.target;
	    ...
	    var elCoord = getCoords(self.node);
	    var shiftX = event.pageX - elCoord.left;
	    var shiftY = event.pageY - elCoord.top;
	...
	Circle.prototype._createNode = function _createNode(idx) {
	  var y = this.coordinates.y - this.radius;
	  var x = this.coordinates.x - this.radius;
